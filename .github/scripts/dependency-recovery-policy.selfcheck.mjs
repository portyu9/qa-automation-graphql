...
function job({
  ...
  return {
    ...
    steps: [
      ...
      { name: step, conclusion, started_at: startedAt, completed_at: completedAt },
    ],
  };
}
...
function gate(name = 'ci-gate', conclusion = 'failure') {
  return {
    ...
    steps: [
      {
        name: name === 'security-gate' ? 'Evaluate security jobs' : 'Evaluate CI jobs',
        conclusion,
...
function canonicalFixture() {
  ...
  const pull = {
    ...
    head: {
      ref: 'dependabot/npm_and_yarn/graphql-runtime',
...
function runFixture({
  ...
  headSha = 'b'.repeat(40),
  headBranch = 'dependabot/npm_and_yarn/graphql-runtime',
} = {}) {
  return {
    id: 501,
...
const substantiveSteps = [
  'Validate repository and documentation contracts',
  'Type check',
  'Run governed tests with coverage',
  'Validate test and coverage evidence',
  'Validate schema contract and fingerprint',
  'Validate committed operation manifest',
  'Bind TypeScript API surface to Node 22',
  'Node 22 compatibility contracts',
  'Generate npm audit evidence',
...
test('recovery config is bounded and GraphQL infrastructure-only', () => {
  ...
  assert.deepEqual(recoveryConfig.transientSteps, [
    'Pin npm runtime',
    'Install locked dependency graph',
    'Pin npm runtime on Node 22',
    'Pin and verify npm runtime',
    'Upload GraphQL qualification evidence',
  ]);
  ...
  assert.ok(
    validateRecoveryConfig({
      ...recoveryConfig,
      transientSteps: [...recoveryConfig.transientSteps, 'Validate schema contract and fingerprint'],
    }).length > 0,
  );
});
...
test('signature model is narrow and deterministic evidence outranks transient words', () => {
  ...
  assert.deepEqual(matchingTransientSignatures('502 schema errors found'), []);
...
test('deterministic blocker wins inside an otherwise transient failed step', () => {
  const result = classifyLeafJobFailure(
    job(),
...
test('GraphQL, schema, compatibility, validation, and security semantics never become transient', () => {
...
test('injected-client orchestration requests exactly one canonical transient rerun', async () => {
  ...
  const transientJob = job({ id: 10, step: 'Install locked dependency graph' });
